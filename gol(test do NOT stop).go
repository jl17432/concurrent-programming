...
func buildWorld(world [][]byte, index, subHeight, imageWidth int,numOfWorker int) [][]byte{
	...
	if index == 0{
		for i := 0; i < imageWidth; i++{
			workPlace [0][i] = world[len(world) - 1][i]
		}
	}else{
		for i := 0; i < imageWidth; i++{
			workPlace [0][i] = world[index * subHeight - 1][i]
		}
	}



	if index == numOfWorker - 1{
		for i := 0; i < imageWidth; i++{
			workPlace [subHeight + 1][i] = world[0][i]
		}
	}else{
		for i := 0; i < imageWidth; i++{
			workPlace [subHeight + 1][i] = world [(index + 1) * subHeight][i]
		}
	}
	...
	for y := 1; y < subHeight - 1; y++{
		for x := 0; x < imageWidth; x++{
			workPlace [y][x] = world[index * subHeight + y][x]
		}
	}
	return workPlace
}
