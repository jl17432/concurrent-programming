...
func buildWorld(world [][]byte, index, subHeight, imageWidth int,numOfWorker int) [][]byte{
	...
	end := (index + 1) * subHeight                                  // lower bound can be larger than the total height
...
func worker(world [][]byte, subHeight, imageWidth int,out chan<- [][]byte){
	nextPart := make([][]byte, subHeight)
	for i := range nextPart{
		nextPart[i] = make([]byte, imageWidth)
	}
		...
		for x := 0; x < imageWidth; x++ {
			...
			for i := -1; i < 2; i++{
				for j := -1; j < 2; j++{
					...
					if world[y + i][(x + j + imageWidth) % imageWidth] == 255{                  //if there is an alive neighbour, the count of alive neighbours increase by 1
						aliveNeighbours += 1
					}
				}
			}
			if world[y][x] == 255{
				if aliveNeighbours < 2 || aliveNeighbours > 3{                  //if the cell itself is alive, check the neighbours:
					nextPart[y][x] = 0                                         //if it has <2 or>3 alive neighbours, it will die in nextWorld :(
				} else{nextPart[y][x] = 255}                                   //if it has =2 or =3 alive neighbours, it will survive in nextWorld :)
			}
			if world[y][x] == 0{
				if aliveNeighbours == 3{                                        //if the cell itself is dead, check the neighbours:
					nextPart[y][x] = 255                                       //if it has =3 neighbours, it will become alive in nextWorld ;)
				}else{nextPart[y][x] = 0}
			}
		}
	}
...
func distributor(p golParams, d distributorChans, alive chan []cell) {
	...
	numOfWorkers := p.threads
	subHeight := p.imageHeight/numOfWorkers
	out := make([]chan [][]byte, numOfWorkers)
	for i := range out{
	...
	for turns := 0; turns < p.turns; turns++ {
		for i := 0; i <= numOfWorkers; i++{
			workPlace := buildWorld(world,i,subHeight,p.imageWidth,numOfWorkers)
			go worker(workPlace, subHeight, p.imageWidth, out[i])
		}
